    ...
    return(
        <div className="selector-multiple">
            ...
            <div className="selector-multiple-botones">
                <button type="button" onClick={seleccionarTodo} >{'>>'}</button>{/*selecciona todo*/ }
                <button type="button" onClick={deseleccionarTodo} >{'<<'}</button>{/*deselecciona todo*/ }
            </div>
            <ul>
...
interface selectorMultipleProps{
    ...
    onChange(seleccionados: selectorMultipleModel[], 
        noSeleccionados: selectorMultipleModel[]): void;//para responder a los cambios
}
